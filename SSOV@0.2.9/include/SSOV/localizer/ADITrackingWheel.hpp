...
class ADITrackingWheel : public AbstractTrackingWheel {
  private:
    std::shared_ptr<pros::adi::Encoder> encoder;
  ...
  public:
    ...
    encoder(std::make_shared<pros::adi::Encoder>(abs(adi_port), abs(adi_port) + 1, adi_port < 0)) {};

    ADITrackingWheel(int smart_port, int adi_port):
        AbstractTrackingWheel(tpi),
        encoder(std::make_shared<pros::adi::Encoder>(
