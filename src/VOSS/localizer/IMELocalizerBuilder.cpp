...
IMELocalizerBuilder& IMELocalizerBuilder::withTrackWidth(double track_width) {
	if(track_width < 0) {
		...
		errno = EDOM;
		//return PROS_ERR; NOAM HoW dO i ReTuRn An ErRoR
	}
...
IMELocalizerBuilder::withMiddleDistance(double middle_dist) {
 ...
 std::shared_ptr<IMELocalizer> IMELocalizerBuilder::build() {
	...
	return std::make_shared<IMELocalizer>(leftMotors, rightMotors, horizontalMotors, lr_tpi, mid_tpi, track_width, middle_dist);
