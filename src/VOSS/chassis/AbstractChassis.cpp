...
void AbstractChassis::follow(std::vector<Point> path, double max,
                             voss::Flags flags) {
    this->follow(std::move(path), default_controller, default_ec, max, flags);
}
void AbstractChassis::follow(std::vector<Point> path, controller_ptr controller,
                             double max, voss::Flags flags) {
    this->follow(std::move(path), std::move(controller), default_ec, max,
                 flags);
}
void AbstractChassis::follow(std::vector<Point> path, controller_ptr controller,
                             ec_ptr ec, double max, voss::Flags flags) {
    ...
    Point endPt = path.at(path.size() - 1);
    controller->set_target({endPt.x, endPt.y}, flags & voss::Flags::RELATIVE,
                           ec);
