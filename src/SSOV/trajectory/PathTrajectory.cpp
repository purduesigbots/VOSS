...
PathTrajectory::PathTrajectory(std::shared_ptr<Path> path, const TrajectoryConstraints& constraints): path(path) {
    ...
    profile_samples.reserve(path_samples.distances.size());
    ...
    profile = MotionProfile(profile_samples);
}

TrajectoryState PathTrajectory::at(double t) {
    MotionState state = profile.at(t);
    PoseWithCurvature pose = path->at(state.pos);
