...
void BoomerangController::turn(double target, double max, double exit_error, MoveFlags flags) {
    mode = ANGULAR;
...
std::array<double, 2> BoomerangController::translational() {
    ...
    if (no_pose) {
        ...
        carrot_point = {linear_target[0] - lin_error * cos(angular_target) * lead_pct,
