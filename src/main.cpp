...
auto [left, right] = chassis.getMotors();
...
void opcontrol() {
    ...
    while (true) {
        ...
        printf("left: %.2f, right: %.2f\n", left->get_actual_velocity(),
               right->get_actual_velocity());

        if (master.get_digital_new_press(DIGITAL_Y)) {
            odom->set_pose(voss::Pose{0.0, 0.0, 180});
            chassis.move(voss::Pose{-20, 0, 0}, pid->modify_exit_error(4), 100,
                         voss::Flags::RELATIVE | voss::Flags::REVERSE |
                             voss::Flags::THRU);
            chassis.turn(0, swing->modify_angular_exit_error(4), 100,
                         voss::Flags::THRU, voss::AngularDirection::CLOCKWISE);
            chassis.move(voss::Pose{50, 0, 0}, boomerang->modify_exit_error(4),
                         100, voss::Flags::THRU);
            chassis.turn(135, pid->modify_angular_exit_error(4), 100,
                         voss::Flags::THRU,
                         voss::AngularDirection::COUNTERCLOCKWISE);
            chassis.move({0, 0, 180}, boomerang);
        }
